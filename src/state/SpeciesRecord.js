...
const SpeciesRecord = ({ data }) => {
        ...
        if (data.scientificName) {
            axios.get(`https://en.wikipedia.org/w/api.php?action=query&origin=*&titles=${data.scientificName}&prop=extracts|info|pageimages&format=json&exintro=1&inprop=url&pithumbsize=250`)
                .then(resp => {
                    ...
                    if (page.extract) {
                        setWikipediaExtract(page.extract + `<p>Source: <a href="${page.fullurl}">${page.title}</a> on Wikipedia</p?`);
                    }
                    if (page.thumbnail) {
    ...
    return (
        <div className="panel">
            ...
            {data.speciesGlobal.otherCommonNames && <div><b>Other common names:</b><ul>{data.speciesGlobal.otherCommonNames.map((name, i) => {return <li key={i}>{name}</li>})}</ul></div>}
